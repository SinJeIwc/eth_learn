...
export default function AuthModal({ onLogin, onLogout, isAuthenticated, userData }: AuthModalProps) {
  ...
  const handleXsollaAuth = () => {
    ...
    try {
      // Use Xsolla Login Widget URL with Login ID (not Project ID!)
      const loginUrl = new URL('https://login-widget.xsolla.com/latest/');
      loginUrl.searchParams.append('projectId', XSOLLA_CONFIG.LOGIN_ID);
        ...
        <div className="space-y-4">
          <div className="text-center mb-4">
            ...
            <p className="text-gray-400 text-sm">
              Используйте ваш аккаунт Xsolla для входа
            </p>
          </div>
          ...
          {!XSOLLA_CONFIG.PROJECT_ID || XSOLLA_CONFIG.DEMO_MODE ? (
            <div className="bg-yellow-900/20 border border-yellow-500 text-yellow-400 p-4 rounded-lg text-sm">
              ...
              <p className="text-xs">
                Для использования Xsolla необходимо:
              </p>
              <ol className="text-xs list-decimal list-inside mt-2 space-y-1">
                <li>Создать аккаунт на <a href="https://publisher.xsolla.com/" target="_blank" rel="noopener noreferrer" className="underline">publisher.xsolla.com</a></li>
          ...
          <p className="text-xs text-gray-500 text-center">
            Безопасная аутентификация через Xsolla OAuth 2.0
          </p>
        </div>
      )}
        ...
        <form onSubmit={handleXsollaIdAuth} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              ID можно найти в профиле Xsolla
            </p>
          </div>
