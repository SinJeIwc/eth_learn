  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-gradient-to-r from-purple-500 to-indigo-600 text-white p-5 rounded-xl mb-6 flex justify-around flex-wrap gap-4">
        ...
        <div><strong>Balance:</strong> 💰 {parseFloat(balance).toFixed(2)} FGOLD</div>
        <div><strong>Plots:</strong> {player.plotCount}</div>
      </div>
          ...
          {plots.map(plot => (
            <div
              ...
              className={`border-4 rounded-xl p-4 min-h-[200px] flex flex-col items-center justify-center ${
                plot.harvestTime === 0 ? 'border-green-300 bg-green-50' :
                plot.canHarvest ? 'border-yellow-400 bg-yellow-50' :
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {seeds.map(seed => (
            <div key={seed.id} className="border-2 rounded-lg p-4 bg-white">
              ...
              {inventory[seed.id] && (
                <p className="text-sm text-green-600">In stock: {inventory[seed.id]}</p>
              )}
              <button
