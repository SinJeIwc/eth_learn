        ...
        <div className="relative p-6 rounded-lg ring-2 ring-blue-700/50 shadow-xl">
          ...
          <div className="relative z-10">
            ...
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-2xl font-bold text-white font-pixelify-sans">BACKPACK</h2>
              <button
                onClick={onClose}
                className="w-8 h-8 grid place-items-center hover:opacity-75 transition-opacity"
                aria-label="Close"
              >
                <Image src={UI_PATHS.CLOSE} alt="Close" width={24} height={24} className="object-contain" />
              </button>
            </div>
            {/* Inventory Items */}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="text-white font-bold font-pixelify-sans">{invItem.item.name}</div>
                        <div className="text-gray-200 text-xs">
                          {invItem.item.type === "seed" ? "🌱 Seed" : "🌾 Crop"}
                        </div>
                        <div className="text-gray-200 text-xs">Quantity: {invItem.quantity}</div>
                      </div>
                    </div>
                  </div>
                ))
            ...
            {selectedItem && (
              <div className="bg-black/30 p-4 rounded mb-2">
                <h3 className="text-white font-bold mb-2 font-pixelify-sans">{selectedItem.item.name}</h3>
                <p className="text-gray-100/90 text-sm mb-3">{selectedItem.item.description}</p>

                <div className="mb-3">
                  <label className="text-white text-sm block mb-1">Quantity:</label>
                  <input
                    type="number"
                    min="1"
